...
export const Container = styled.div<{ left?: number; top?: number }>`
  left: ${(props) => (props.left || props.left === 0 ? `${props.left}px` : '50%')};
  position: fixed;
  top: ${(props) => (props.top || props.top === 0 ? `${props.top}px` : '50%')};
  transform: ${(props) =>
    !props.left && props.left !== 0 && props.top !== 0 && !props.top && 'translate(-50%, -50%)'};
  z-index: 1000;
`;
